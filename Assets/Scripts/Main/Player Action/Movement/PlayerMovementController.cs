...
public class PlayerMovementController : MonoBehaviour
{
    ...
    private void SynchronizePlatform()
    {
        if (playerMgr.isGround)
        {
            ...
            {
                // 이전 프레임에도 플랫폼 위에 있었다면
                if (transform.parent == platformParent && curPlatform != null)
                {
                    SetCurPlatformNull();
                }
                else
    ...
    private void SetCurPlatformNull()
    {
        if (curPlatform == null)
            return;

        // 물리효과 콜라이더를 다시 플레이어와 충돌하게 설정
        if (curPlatform.TryGetComponent(out SplitMovingPlatform split))
            split.otherSideObject.gameObject.layer = LayerMask.NameToLayer("MovingPlatformPhysics");

        curPlatform = null;
        transform.parent = null;
    }

    private void SetCurPlatform()
    {
        // 물리효과 콜라이더는 플레이어와 충돌하지 않게 설정
        if (curPlatform.TryGetComponent(out SplitMovingPlatform split))
            split.otherSideObject.layer = LayerMask.NameToLayer("MovingPlatformGhost");

        curPlatform = playerMgr.curGroundCollider;
    }

    private void SetPlatformParentPosition()
    {
